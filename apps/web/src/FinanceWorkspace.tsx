...
const accountTypes = Object.keys(accountLabels) as AccountType[];
...
export function FinanceWorkspace({ session, defaultCurrency, profile, onProfileSaved, onSignOut }: { session: Session; defaultCurrency: string; profile: Profile | null; onProfileSaved: (profile: Profile) => void; onSignOut: () => void }) {
  ...
  const orderedAccounts = useMemo(() => accounts.filter((account) => visibleAccountTypes.includes(account.account_type)).sort((left, right) => left.name.localeCompare(right.name)), [accounts, visibleAccountTypes]);
...
function AccountForm({ currency, busy, onSubmit, onCancel }: { currency: string; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void }) {
  ...
  return <form className="finance-form progressive-form" onSubmit={onSubmit}><div className="form-section"><p className="form-section-title">Lo esencial</p><label>Nombre<input name="name" maxLength={100} required autoFocus placeholder="Cuenta principal" /></label><label>Tipo<select name="type" value={type} onChange={(event) => setType(event.target.value as AccountType)}>{Object.entries(accountLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>Moneda<CurrencySelector value={currency} /></label></div>{canChooseColor && <details className="form-advanced"><summary>Personalizar apariencia</summary><div><label>Color de {type === "bank" ? "la cuenta" : "la tarjeta"}<select name="color" defaultValue="emerald"><option value="emerald">Verde</option><option value="blue">Azul</option><option value="violet">Violeta</option><option value="rose">Rosa</option></select></label><p>Este color te ayuda a reconocer la cuenta visualmente; no altera sus movimientos.</p></div></details>}<div className="dialog-actions"><button type="button" className="text-button" onClick={onCancel}>Cancelar</button><button className="primary-button" disabled={busy}>{busy ? "Guardando…" : "Crear cuenta"}</button></div></form>;
}
...
function TransactionForm({ accounts, categories, busy, preset, onOpenLibrary, onSubmit, onCancel }: { accounts: FinancialAccount[]; categories: Category[]; busy: boolean; preset: TransactionLibraryItem | null; onOpenLibrary: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void }) {
  ...
  const initialCategory = categories.find((item) => item.id === preset?.category_id);
  const [categoryId, setCategoryId] = useState(() => initialCategory?.parent_id ?? initialCategory?.id ?? "");
  ...
  const rootCategories = validCategories.filter((category) => !category.parent_id);
  const subcategories = validCategories.filter((category) => category.parent_id === categoryId);
  const categoryValue = subcategoryId || categoryId;
  return <form className="finance-form quick-movement-form progressive-form" onSubmit={onSubmit}><input type="hidden" name="type" value={type}/><div className="movement-form-toolbar"><span>{preset ? `Plantilla: ${preset.description}` : "Registro rápido"}</span><button className="secondary-button" type="button" onClick={onOpenLibrary}><WalletCards size={16} /> Elegir de biblioteca</button></div><div className="movement-type-picker" role="group" aria-label="Tipo de movimiento">{(["expense","income","transfer","adjustment"] as const).map((value)=><button key={value} type="button" className={type===value?"is-active":""} onClick={()=>{setType(value);setCategoryId("");setSubcategoryId("");}}>{transactionLabels[value]}</button>)}</div><div className="form-section"><p className="form-section-title">Movimiento</p><label>Concepto<input name="description" maxLength={240} required autoFocus defaultValue={preset?.description ?? ""} placeholder="¿Qué movimiento quieres registrar?" /></label><label>Importe<input name="amount" type="number" min="0.0001" step="0.0001" inputMode="decimal" defaultValue={preset?.default_amount ?? ""} required /></label><label>Cuenta<select name="account" value={sourceId} onChange={(event) => {setSourceId(event.target.value);window.localStorage.setItem("financiera.last-account",event.target.value);}} required>{accounts.map((account) => <option value={account.id} key={account.id}>{account.name} · {account.currency_code}</option>)}</select></label>{type === "transfer" && <label>Cuenta de destino<select name="destination" required>{destinations.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select>{!destinations.length && <small>Necesitas otra cuenta activa en {source?.currency_code}.</small>}</label>}{type === "adjustment" && <label>Dirección<select name="direction"><option value="credit">Aumentar saldo</option><option value="debit">Reducir saldo</option></select></label>}</div>{(type === "expense" || type === "income") && <div className="form-section form-section--dependent"><p className="form-section-title">Clasificación <span>opcional</span></p><label>Categoría<select value={categoryId} onChange={(event) => {setCategoryId(event.target.value);setSubcategoryId("");}}><option value="">Sin categoría</option>{rootCategories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label><label>Subcategoría{(!categoryId || !subcategories.length) && <input type="hidden" name="category" value={categoryValue} />}<select name="category" value={categoryValue} disabled={!categoryId || !subcategories.length} onChange={(event) => setSubcategoryId(event.target.value)}><option value={categoryId}>{!categoryId ? "Elige primero una categoría" : !subcategories.length ? "Esta categoría no tiene subcategorías" : "Sin subcategoría"}</option>{subcategories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select>{categoryId && !subcategories.length && <small>La categoría elegida se aplicará directamente.</small>}</label></div>}<details className="form-advanced"><summary>Más opciones</summary><div><label>Fecha<input type="date" name="date" defaultValue={new Date().toISOString().slice(0, 10)} required /></label>{(type === "expense" || type === "income") && <label className="checkbox-field"><input type="checkbox" name="save-to-library" />Guardar en mi biblioteca para reutilizarlo</label>}</div></details><div className="dialog-actions"><button type="button" className="text-button" onClick={onCancel}>Cancelar</button><button className="primary-button" disabled={busy || (type === "transfer" && !destinations.length)}>{busy ? "Registrando…" : "Registrar movimiento"}</button></div></form>;
}
