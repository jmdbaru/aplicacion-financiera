...
export function FinanceWorkspace({ session, defaultCurrency, profile, onProfileSaved, onSignOut }: { session: Session; defaultCurrency: string; profile: Profile | null; onProfileSaved: (profile: Profile) => void; onSignOut: () => void }) {
  ...
  useEffect(() => {
    if (view !== "transactions" || transactionPanel !== "library") return;
  ...
  }, [session, view, transactionPanel, libraryType, libraryPage]);
  useEffect(() => {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-is-collapsed" : ""}`}>
    ...
    <main id="main-content" className="main-content"><button className="floating-create" type="button" aria-keyshortcuts="N" aria-label={activeAccounts.length ? "Añadir movimiento" : "Crear cuenta"} title={activeAccounts.length ? "Nuevo movimiento · tecla N" : "Crear cuenta"} onClick={() => { setTransactionPreset(null); setDialog(activeAccounts.length ? "transaction" : "account"); }}><Plus size={25} /><span className="sr-only">{activeAccounts.length ? "Movimiento" : "Cuenta"}</span></button>
      ...
      {loading ? <LoadingState /> : <AnimatePresence mode="wait" initial={false}><motion.div className="workspace-stage" key={view} initial={reduceMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: -3 }} transition={{ duration: 0.16, ease: "easeOut" }}>
        ...
        {view === "accounts" && <>{accountFiltersOpen && <div className="quick-filters filter-reveal"><span>Ver:</span><select aria-label="Filtrar tipo de cuenta" value={accountTypeFilter} onChange={(event) => setAccountTypeFilter(event.target.value as "" | AccountType)}><option value="">Todas las categorías</option>{Object.entries(accountLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span>Ordenar por:</span><select value={accountOrder} onChange={(event) => setAccountOrder(event.target.value as "name" | "balance" | "type")}><option value="name">Nombre</option><option value="balance">Saldo</option><option value="type">Tipo</option></select></div>}{accountNotice && <p className="account-toast" role="status">{accountNotice}</p>}<AccountsView accounts={orderedAccounts} busy={busy} filtersOpen={accountFiltersOpen} onToggleFilters={() => setAccountFiltersOpen((value) => !value)} onCreate={() => setDialog("account")} onOpen={openAccountDetail} onToggle={(account) => void runAction(async () => { await setAccountActive(session, account.id, !account.is_active); await refresh(); }, "No se pudo cambiar el estado de la cuenta.")} onDelete={(account) => void (async () => { try { await deleteAccount(session, account.id); await refresh(); } catch { setAccountNotice("No podemos borrar esta cuenta porque conserva operaciones o configuraciones vinculadas. Puedes archivarla para mantener tu historial."); window.setTimeout(() => setAccountNotice(""), 1000); } })()} /></>}
    ...
    {dialog && <ModalFrame title={dialog === "account" ? "Nueva cuenta" : "Nuevo movimiento"} onClose={() => { setTransactionPreset(null); setDialog(null); }} labelledBy="finance-dialog-title">{dialog === "account" ? <AccountForm currency={defaultCurrency} busy={busy} onSubmit={submitAccount} onCancel={() => setDialog(null)} /> : <TransactionForm key={transactionPreset?.id ?? "new"} accounts={activeAccounts} categories={activeCategories} busy={busy} preset={transactionPreset} onSubmit={submitTransaction} onCancel={() => { setTransactionPreset(null); setDialog(null); }} />}</ModalFrame>}
...
function AccountsView({ accounts, busy, filtersOpen, onToggleFilters, onCreate, onOpen, onToggle, onDelete }: { accounts: FinancialAccount[]; busy: boolean; filtersOpen: boolean; onToggleFilters: () => void; onCreate: () => void; onOpen: (account: FinancialAccount) => void; onToggle: (account: FinancialAccount) => void; onDelete: (account: FinancialAccount) => void }) {
...
function TransactionsView({ movements, transactions, count, page, search, dateFrom, dateTo, filtersOpen, categoryNames, accountNames, canCreate, onCreate, onToggleFilters, onSearch, onDateFrom, onDateTo, onPage, onReverse }: { movements: Movement[]; transactions: LedgerTransaction[]; count: number; page: number; search: string; dateFrom: string; dateTo: string; filtersOpen: boolean; categoryNames: Map<string, string>; accountNames: Map<string, string>; canCreate: boolean; onCreate: () => void; onToggleFilters: () => void; onSearch: (value: string) => void; onDateFrom: (value: string) => void; onDateTo: (value: string) => void; onPage: (value: number | ((current: number) => number)) => void; onReverse: (id: string) => void }) {
...
function TransactionForm({ accounts, categories, busy, preset, onSubmit, onCancel }: { accounts: FinancialAccount[]; categories: Category[]; busy: boolean; preset: TransactionLibraryItem | null; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void }) {
