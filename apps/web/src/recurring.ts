...
export type RecurringRule = { id: string; name: string; transaction_type: "income" | "expense" | "transfer" | "adjustment"; account_id: string; destination_account_id: string | null; category_id: string | null; currency_code: string; amount: number; frequency: "daily" | "weekly" | "monthly"; interval_count: number; weekday: number | null; monthly_day: number | null; next_run_on: string; end_on: string | null; time_zone: string; is_active: boolean };
...
export async function setRecurringActive(session: Session, ruleId: string, isActive: boolean) { const { error } = await client().from("recurring_rules").update({ is_active: isActive }).eq("id", ruleId).eq("user_id", session.user.id); if (error) throw error; }
export async function deleteRecurringRule(session: Session, ruleId: string) { const { error } = await client().from("recurring_rules").delete().eq("id", ruleId).eq("user_id", session.user.id); if (error) throw error; }
