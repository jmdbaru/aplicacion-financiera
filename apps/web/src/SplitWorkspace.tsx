...
export function SplitWorkspace({ session, currency }: { session: Session; currency: string }) {
  ...
  return <section>
    ...
    {historyOpen && <ModalFrame title="Historial de eventos" onClose={() => setHistoryOpen(false)} labelledBy="split-history-title"><div className="split-history-list">{events.length ? events.map((item) => <button type="button" key={item.id} onClick={() => void openEvent(item)}><span><strong>{item.name}</strong><small>{item.event_type === "trip" ? "Viaje" : item.event_type === "home" ? "Casa" : "Evento"} · {item.currency_code}</small></span><span>{new Date(item.created_at).toLocaleDateString("es-ES")}</span></button>) : <p className="ux-hint">Todavía no hay eventos creados.</p>}</div></ModalFrame>}
  </section>;
}
