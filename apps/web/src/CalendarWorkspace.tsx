...
const dayTone = (items: LedgerTransaction[]) => {
  const hasExpense = items.some((item) => item.transaction_type === "expense");
  const hasIncome = items.some((item) => item.transaction_type === "income");
  return hasExpense && hasIncome ? "is-mixed" : hasExpense ? "is-expense" : hasIncome ? "is-income" : "";
};
